...
import { Select, Option, Label, FlexBox, FlexBoxAlignItems, FlexBoxJustifyContent } from '@ui5/webcomponents-react'
...
export default function ThemeSelect() {
  const [theme, setThemeState] = useState<string>(() => localStorage.getItem(STORAGE_KEY) || THEMES[0].value)
  ...
  const selectedIndex = useMemo(
    () => Math.max(0, THEMES.findIndex(t => t.value === theme)),
    [theme]
  )
  ...
  return (
    ...
    >
      ...
      >
        {THEMES.map((t, idx) => (
          <Option key={t.value} data-value={t.value} /* @ts-ignore */ selected={idx === selectedIndex}>
            {t.label}
          </Option>
        ))}
      </Select>
